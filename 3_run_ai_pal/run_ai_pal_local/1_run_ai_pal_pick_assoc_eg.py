...
# Copy the best training checkpoints into input; specify exact files below.
...
def main():
    ...
    if not selected_picker_pos_neg:
    ...
    if missing_picker_pos_neg:
        raise ValueError(
            "selected picker specifications are missing: POS_NEG={}"
            .format(sorted(missing_picker_pos_neg))
        )
    picker_specs = {}
    ...
    print(
        "selected pickers: continuous POS_NEG={} | "
        "postprocess POS_NEG={} POS={}".format(
            selected_picker_pos_neg,
