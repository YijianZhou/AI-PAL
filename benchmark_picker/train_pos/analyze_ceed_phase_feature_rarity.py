"""Analyze CEED station-waveform rarity and build training phase files.

Input is the PAL-style CEED phase file produced by extract_ceed_phase.py:

    ot,lat,lon,dep,mag
    net.sta.loc.chn,tp,ts,dist_km

The pick-line distance is epicentral distance. This script computes the third
rarity feature as hypocentral distance:

    hypo_dist_km = sqrt(dist_km ** 2 + dep_km ** 2)
...
hypocentral distance. The train/valid split is station-level: every sample from
the same net.sta station is assigned to the same split by default.

Main outputs:
  * feature CSV with one row per station waveform
  * PDF figure of feature distributions
  * PDF figure of joint probability, rarity, and num_aug divisions
  * CSV training phase table, one station waveform per row
  * PAL-like grouped training phase file with per-pick split/num_aug metadata
"""
...
# USER SETTINGS
# =============================================================================
PHASE_IN = "output/ceed_phase.pha"
FEATURE_CSV_OUT = "output/ceed_phase_station_feature_rarity.csv"
TRAINING_CSV_OUT = "output/ceed_phase_train_augmented.csv"
TRAINING_PHA_OUT = "output/ceed_phase_train_augmented.pha"
SUMMARY_OUT = "output/ceed_phase_train_augmented_summary.csv"
FEATURE_FIG_OUT = "output/ceed_phase_feature_distributions.jpg"
PROB_FIG_OUT = "output/ceed_phase_rarity_distribution.jpg"
PHASE_REQUIREMENT = "both"  # both, p, s, or any
MAX_HYPO_DIST_KM = 200.0
FIGURE_DPI = 600
STATION_SPLIT_LEVEL = "net_sta"  # net_sta or net_sta_loc_chn
VALIDATION_FRACTION = 0.10
RANDOM_SEED = 20250630
AUGMENTATION_VALUES = [1, 2, 3, 4]
RARITY_PERCENTILES = [50, 75, 90]
VALIDATION_NUM_AUG = 1
MAG_BIN_WIDTH = 0.1
HYPO_DIST_BIN_WIDTH = 10.0
MAG_HIST_SMOOTH_SIGMA_BINS = 1.0
HYPO_DIST_HIST_SMOOTH_SIGMA_BINS = 1.5
SPATIAL_BIN_KM = 5.0
SPATIOTEMPORAL_TIME_BIN_DAYS = 10.0
SPATIOTEMPORAL_COUNT_LOG_BIN_WIDTH = 0.1
SPATIOTEMPORAL_COUNT_HIST_SMOOTH_SIGMA_BINS = 1.5
...
def read_ceed_phase(path, phase_requirement):
    samples = []
    event_headers = []
    current = None
    counts = Counter()
...
def filter_samples_by_hypo_distance(samples, max_hypo_dist_km, counts):
    ...
    dep = np.array([s["dep"] for s in samples], dtype=float)
...
def write_summary(path, args, counts, split_by_key, thresholds, samples):
    ...
    with open(path, "w", newline="") as fp:
        ...
        writer.writerow(["phase_pick_distance_column", "epi_dist_km"])
        writer.writerow(["computed_distance_feature", "hypo_dist_km=sqrt(epi_dist_km^2+dep_km^2)"])
...
def build_settings():
    ...
    args = SimpleNamespace(
        phase_in=PHASE_IN, feature_csv_out=FEATURE_CSV_OUT,
        training_csv_out=TRAINING_CSV_OUT, training_pha_out=TRAINING_PHA_OUT,
        summary_out=SUMMARY_OUT, feature_fig_out=FEATURE_FIG_OUT,
        prob_fig_out=PROB_FIG_OUT, phase_requirement=PHASE_REQUIREMENT,
        max_hypo_dist_km=MAX_HYPO_DIST_KM, figure_dpi=FIGURE_DPI,
        station_split_level=STATION_SPLIT_LEVEL,
        validation_fraction=VALIDATION_FRACTION, random_seed=RANDOM_SEED,
        augmentation_values=AUGMENTATION_VALUES, rarity_percentiles=RARITY_PERCENTILES,
        validation_num_aug=VALIDATION_NUM_AUG, mag_bin_width=MAG_BIN_WIDTH,
        hypo_dist_bin_width=HYPO_DIST_BIN_WIDTH,
        mag_hist_smooth_sigma_bins=MAG_HIST_SMOOTH_SIGMA_BINS,
        hypo_dist_hist_smooth_sigma_bins=HYPO_DIST_HIST_SMOOTH_SIGMA_BINS,
        spatial_bin_km=SPATIAL_BIN_KM,
        spatiotemporal_time_bin_days=SPATIOTEMPORAL_TIME_BIN_DAYS,
        spatiotemporal_count_log_bin_width=SPATIOTEMPORAL_COUNT_LOG_BIN_WIDTH,
        spatiotemporal_count_hist_smooth_sigma_bins=SPATIOTEMPORAL_COUNT_HIST_SMOOTH_SIGMA_BINS,
...
def main():
    args = build_settings()
    event_headers, samples, counts = read_ceed_phase(args.phase_in, args.phase_requirement)
    ...
    print("phase pick-line distance column: epi_dist_km")
    print("rarity distance feature: hypo_dist_km = sqrt(epi_dist_km^2 + dep_km^2)")
