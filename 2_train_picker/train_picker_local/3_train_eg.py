"""Train configured AI-PAL picker models from one shared Zarr dataset."""
...
for model_name in ENABLED_MODELS:
    ...
    for path in (src_dir, config_path, train_script):
        if not path.exists():
            raise FileNotFoundError(path)
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(config_path, src_dir / "config.py")
    print(
        "training {} from {} -> {}".format(model_name, ZARR_PATH, ckpt_dir),
        flush=True,
    )
