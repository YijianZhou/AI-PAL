"""Main function for offline stream picking with Frame Transformer.

The Frame Transformer model is loaded once in the main process.  Parallelism is handled
with threads so all workers share the same GPU-resident model parameters,
matching realtime picking behavior and avoiding one model copy per worker.
"""
import argparse
import os
import warnings
...
def pick_one_day(date, run_picker, data_dir, sta_dict, out_root):
...
def main():
    ...
    run_picker = picker_module.FT_Picker(args.ckpt_dir, args.ckpt_idx, args.gpu_idx)
    ...
    if num_workers == 1:
        for idx, date in enumerate(date_list, start=1):
            pick_path = pick_one_day(date, run_picker, args.data_dir, sta_dict, args.out_root)
