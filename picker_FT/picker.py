...
step_len = cfg.ft_frame_length
step_len_npts = int(step_len * samp_rate)
step_stride = cfg.ft_frame_step
step_stride_npts = int(step_stride * samp_rate)
num_steps = int((win_len - step_len) / step_stride) + 1
...
class FT_Picker(object):
  """ Frame Transformer picker for raw stream data
  """
  def __init__(self, ckpt_dir, ckpt_idx=-1, gpu_idx=0):
    if os.path.isfile(ckpt_dir):
        ...
        if int(ckpt_idx)==-1:
            ...
            ckpt_path = sorted(glob.glob(os.path.join(ckpt_dir, '%s_*.ckpt'%ckpt_idx)))[0]
    print('FT checkpoint: %s'%ckpt_path)
    # load model
    gpu_idx = int(gpu_idx)
    if gpu_idx < 0:
  ...
  def pick(self, stream, fout=None, pick_start_time=None, pick_end_time=None,
    ...
    # 2. run SAR picker
    picks_raw = self.run_ft(st_data_cuda, start_time, num_win, miss_chn)
    ...
    for consensus in picks_raw:
        ...
        output.update(
            ...
            s_amp=s_amp,
            sources=['FT'],
            picker_cluster_sizes={'FT': int(consensus['num_support'])},
        )
        picks.append(output)
        if fout:
  ...
  def run_ft(self, st_data_cuda, start_time, num_win, miss_chn):
    print('2. run Frame Transformer for phase picking')
    t = time.time()
    ...
    for batch_idx in range(num_batch):
        ...
        win_data = self.prepare_windows(st_win_cuda, win_idx_list, miss_chn)
        with torch.inference_mode():
            with torch.autocast(
                ...
                pred_logits = self.model(win_data)
            pred_probs = F.softmax(pred_logits.float(), dim=1).transpose(1, 2).cpu().numpy()
        # decode to sec
    ...
    print('  {} raw P&S picks | FT run time {:.2f}s'.format(len(picks_raw), time.time()-t))
    return np.array(picks_raw, dtype=dtype)

  def prepare_windows(self, st_win_cuda, win_idx_list, miss_chn):
