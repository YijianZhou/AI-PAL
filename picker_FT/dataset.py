"""Positive/negative lazy Zarr dataset for Frame Transformer training."""
import os
import zarr
import numpy as np
...
class Positive_Negative(Dataset):
  def __init__(self, zarr_path, zarr_group):
    ...
    self.pos_target_path = os.path.join(zarr_path, zarr_group, 'positive_target_frame')
    self.neg_data_path = os.path.join(zarr_path, zarr_group, 'negative_data')
    self.neg_target_path = os.path.join(zarr_path, zarr_group, 'negative_target_frame')
  ...
  def _open(self):
    if self.pos_data is None:
        ...
        if self.pos_target.ndim != 2 or self.neg_target.ndim != 2:
            raise ValueError(
                'Expected integer frame targets [N, num_frames], got {} and {}'.format(
                    self.pos_target.shape, self.neg_target.shape
                )
            )
...
class PositiveOnly(Dataset):
  ...
  def __init__(self, zarr_path, zarr_group):
    ...
    self.target_path = os.path.join(
        zarr_path, zarr_group, 'positive_target_frame'
    )
