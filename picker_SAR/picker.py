...
class SAR_Picker(object):
  ...
  def __init__(self, ckpt_dir, ckpt_idx=-1, gpu_idx=0):
    if os.path.isfile(ckpt_dir):
        ...
        if int(ckpt_idx)==-1:
            ...
            ckpt_path = sorted(glob.glob(os.path.join(ckpt_dir, '%s_*.ckpt'%ckpt_idx)))[0]
    print('SAR checkpoint: %s'%ckpt_path)
    # load model
    gpu_idx = int(gpu_idx)
    if gpu_idx < 0:
  ...
  def pick(self, stream, fout=None, pick_start_time=None, pick_end_time=None,
    ...
    # 3.1 ensemble sliding-window picks
    print('3. cluster sliding-window picks')
    picks_raw = self.cluster_sliding_window_picks(picks_raw)
    ...
    for consensus in picks_raw:
        ...
        output.update(
            ...
            s_amp=s_amp,
            sources=['SAR'],
            picker_cluster_sizes={'SAR': int(consensus['num_support'])},
        )
        picks.append(output)
        if fout:
  ...
  def run_sar(self, st_data_cuda, start_time, num_win, miss_chn):
    ...
    for batch_idx in range(num_batch):
        ...
        del pred_probs, pred_logits, data_seq
    print('  {} raw P&S picks | SAR run time {:.2f}s'.format(len(picks_raw), time.time()-t))
    return np.array(picks_raw, dtype=dtype)

  def cluster_sliding_window_picks(self, picks_raw):
    """Return median P/S consensus picks supported by distinct windows."""
    consensus = cluster_pair_votes(
      picks_raw, tp_dev, ts_dev,
      min_support=picker_min_cluster_size, source_field='win_idx',
    )
    print('  {} accepted clusters from {} raw picks (minimum {} windows)'.format(
      len(consensus), len(picks_raw), picker_min_cluster_size
    ))
    return consensus
