...
num_chn = cfg.num_chn
num_steps = getattr(cfg, "rnn_num_steps", 0)
...
def write_cut_progress(out_root, stage, completed_items, total_items,
    ...
    """Publish lightweight progress and volume-usage data for AWS monitoring."""
    ...
    payload = {
        ...
        'bytes_per_sample': int(3 * (win_len + 2) * np.dtype(np.float32).itemsize),
