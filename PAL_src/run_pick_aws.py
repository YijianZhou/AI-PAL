...
def process_day(
    ...
    for day_offset in (-1, 0, 1):
        source_date = observed_date + timedelta(days=day_offset)
        daily_records[day_offset] = cfg.get_data_dict(
            source_date, active, s3_client,
            bucket=s3_bucket, root_prefix=s3_root_prefix,
            location_priority=loc_priority,
        )
    current_records = daily_records[0]
    data_dict = {
        net_sta: sum(
            (daily_records[offset].get(net_sta, []) for offset in (-1, 0, 1)),
            [],
        )
        for net_sta in current_records
    }
    errors = []
    num_processed = 0
