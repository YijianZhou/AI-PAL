"""Buffered daily PAL association, merging, rates, and parallel orchestration."""
...
def input_picks_for_date(cfg, pick_dir, observed_date):
    unique = {}
    for offset in (-1, 0, 1):
...
def write_association_rate_from_picks(
    ...
    """Write station rates from an in-memory daily pick collection."""
    by_station = {}
    ...
    with partial.open("w", newline="", encoding="utf-8") as fp:
        ...
        for station in sorted(by_station):
            station_picks = by_station[station]
            num_picks = len(station_picks)
    ...
    return {
        ...
        "num_station_dates": len(by_station),
    }
...
def write_association_rate(
    ...
    input_picks = input_picks_for_date(cfg, pick_dir, observed_date)
...
def run_daily_association(
    ...
    """Associate target days using picks and raw events from adjacent days."""
    run = AssociationRun(
        subnet_station_files={
    ...
    halo_dates = dates_between(start - timedelta(days=1), end + timedelta(days=1))

    print("stage 1: buffered daily subnet association")
    _require_success(
        "raw association",
        _run_parallel(_raw_worker, _build_raw_tasks(run, halo_dates), run.num_workers),
    )
    print("stage 2: cross-day and cross-subnet canonical merge")
    _require_success(
        "daily merge",
        _run_parallel(
            _merge_day,
            [(run, observed_date) for observed_date in halo_dates],
            run.num_workers,
        ),
    )
