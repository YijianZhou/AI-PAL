"""Run ResUNet positive-window picker on fixed NPY shard datasets."""
import argparse
import json
import os
import re
...
from picker_pos import RUNPositivePicker, configure_positive_picker, rows_to_csv


# Increment when prediction sample-identity handling changes. The workdir
# launcher checks this because it runs a separately installed software copy.
CEED_METADATA_MAPPING_VERSION = 2
...
class NpyShardEventDataset(Dataset):
  ...
  def read_meta_for_shard(self, shard_path, count):
    ...
    if part_path and os.path.exists(part_path):
        ...
        raise FileNotFoundError('No metadata found for shard {}'.format(shard_path))
    if len(df) != count:
        raise ValueError(
            'Metadata row count mismatch for {}: expected {}, found {} in {}'.format(
                shard_path, count, len(df), part_path or self.sample_index
            )
        )
    ...
    rows = pd.to_numeric(df['row_in_shard'], errors='coerce').to_numpy()
    expected = np.arange(count, dtype=np.int64)
    if np.any(~np.isfinite(rows)) or not np.array_equal(rows.astype(np.int64), expected):
        raise ValueError(
            'Invalid row_in_shard sequence for {} in {}'.format(
                shard_path, part_path or self.sample_index
            )
        )
...
def main():
    ...
    picker = RUNPositivePicker(args.ckpt_dir, args.ckpt_idx, args.gpu_idx)
    ...
    print(
        'metadata mapping v{} | expected shards {:,} | samples {:,} | temporary output {}'.format(
            CEED_METADATA_MAPPING_VERSION, expected_shards, expected_samples, partial_file
        ),
        flush=True,
    )
    ...
    try:
        ...
        summary = {
            ...
            'ceed_metadata_mapping_version': CEED_METADATA_MAPPING_VERSION,
            'elapsed_sec': elapsed,
        }
