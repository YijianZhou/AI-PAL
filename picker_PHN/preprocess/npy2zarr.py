"""Add positive/negative PhaseNet sample targets to the shared Zarr."""
import os
import sys
...
from dataset_npy import NpyWindowShards
import warnings
...
def make_loader(sample_npy, num_workers, write_batch_size, prefetch_factor):
...
def write_sequence(zarr_dset, data_loader, chunk_size, compressor, log_interval):
    num_samples = data_loader.dataset.num_samples
    data_shape = (num_samples, num_chn, win_len)
    target_shape = (num_samples, 3, win_len)
    data_out = os.path.join(out_path, zarr_dset+'_data')
    target_out = os.path.join(out_path, zarr_dset+'_target_sample')
    ...
    if write_data:
        ...
        z_data = None
    z_target = zarr.open(target_out, mode='w', shape=target_shape, chunks=(sample_chunk, 3, win_len), dtype=np.float32, compressor=compressor) if write_target else None
    cursor = 0
    t0 = time.time()
    last_t = t0
    for data, target in data_loader:
        ...
        if write_target:
            z_target[cursor:next_cursor] = np.asarray(target, dtype=np.float32)
        write_t = time.time()
        cursor = next_cursor
...
if __name__ == '__main__':
    ...
    parser.add_argument('--write_batch_size', type=int, default=512, help='Kept for CLI compatibility; shard size controls actual batch size.')
    ...
    if not args.positive_only:
        ...
        write_sequence('valid/negative', valid_neg_loader, args.chunk_size, compressor, args.log_interval)
