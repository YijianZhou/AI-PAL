"""Main function for offline stream picking with PhaseNet.

The PhaseNet model is loaded once in the main process.  Parallelism is handled
...
def pick_one_day(date, phn_picker, data_dir, sta_dict, out_root):
...
def main():
    ...
    phn_picker = picker_module.PHN_Picker(args.ckpt_dir, args.ckpt_idx, args.gpu_idx)
    ...
    if num_workers == 1:
        for idx, date in enumerate(date_list, start=1):
            pick_path = pick_one_day(date, phn_picker, args.data_dir, sta_dict, args.out_root)
