"""Run PhaseNet positive-window picker on fixed NPY shard datasets."""
import argparse
import json
import os
import re
...
from picker_pos import PHNPositivePicker, configure_positive_picker, rows_to_csv


METADATA_MAPPING_VERSION = 2
...
class NpyShardEventDataset(Dataset):
  ...
  def read_meta_for_shard(self, shard_path, count):
    ...
    # Always stamp the actual shard being read. Metadata files should already
    # agree, but using the live shard path prevents stale/mismatched paths from
    # corrupting downstream sample keys.
  ...
  def index_part_path(self, shard_path):
    ...
    # CEED shards include two indices in the filename, and the metadata part
    # must use both.  Matching only on shard_id aliases many CEED files onto
    # the first part_*_<shard>.csv and corrupts prediction metadata.
    m_ceed = re.search(r'CEED_file_(\d+)_shard_(\d+)\.npy$', base)
    if m_ceed:
        file_id, shard_id = m_ceed.group(1), m_ceed.group(2)
        candidates.extend([
            os.path.join(self.index_parts_dir, 'part_{}_{}.csv'.format(file_id, shard_id)),
            os.path.join(self.index_parts_dir, 'part_%05d_%06d.csv' % (int(file_id), int(shard_id))),
        ])
    else:
...
def main():
    ...
    picker = PHNPositivePicker(args.ckpt_dir, args.ckpt_idx, args.gpu_idx)
    ...
    print('expected | shards {:,} | samples {:,} | temporary output {}'.format(
    ...
    try:
        ...
        summary = {
            ...
            'metadata_mapping_version': METADATA_MAPPING_VERSION,
            'elapsed_sec': elapsed,
        }
