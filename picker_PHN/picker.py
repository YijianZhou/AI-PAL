...
class PHN_Picker(object):
  """PhaseNet picker for raw stream data
  """
  def __init__(self, ckpt_dir, ckpt_idx=-1, gpu_idx=0):
    ...
    print('PHN checkpoint: %s'%ckpt_path)
    # load model
    gpu_idx = int(gpu_idx)
    if gpu_idx < 0:
  ...
  def pick(self, stream, fout=None, pick_start_time=None, pick_end_time=None,
    ...
    # 2. run PhaseNet picker
    picks_raw = self.run_phn(st_data_cuda, start_time, num_win, miss_chn)
    ...
    for consensus in picks_raw:
        ...
        output.update(
            ...
            s_amp=s_amp,
            sources=['PHN'],
            picker_cluster_sizes={'PHN': int(consensus['num_support'])},
        )
        picks.append(output)
        if fout:
  ...
  def run_phn(self, st_data_cuda, start_time, num_win, miss_chn):
    print('2. run PhaseNet for phase picking')
    t = time.time()
    ...
    for batch_idx in range(num_batch):
        ...
        del pred_probs, pred_logits, data_batch
    print('  {} raw P&S picks | PhaseNet run time {:.2f}s'.format(len(picks_raw), time.time()-t))
