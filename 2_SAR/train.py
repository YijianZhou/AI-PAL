...
def main():
    ...
    valid_loader = DataLoader(valid_set, batch_sampler=valid_sampler, **loader_kwargs)
...
def train_step(model, data, target, neg_ratio, criterion, optimizer):
    ...
    for ii in range(2):
        ...
        acc_seq = pred_i.reshape(-1).eq(tar_pos).sum() / float(tar_pos.size(0))
        if ii == 0:
            acc_list += [acc_seq, num_pos_pred/float(num_pos)]
        if ii == 1:
...
def valid_step(model, data, target, criterion):
    ...
    with torch.no_grad():
        ...
        for ii in range(2):
            ...
            acc_seq = pred_i.reshape(-1).eq(tar_pos).sum() / float(tar_pos.size(0))
            if ii == 0:
                acc_list += [acc_seq, num_pos_pred/float(bs)]
            if ii == 1:
