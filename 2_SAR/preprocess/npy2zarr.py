...
def write_sequence(zarr_dset, sample_npy, chunk_size, compressor, log_interval, args):
    ...
    z_data = open_zarr_array(data_out, data_shape, (sample_chunk, num_chn, win_len), np.float32, compressor, args.zarr_format)
    z_target = open_zarr_array(target_out, target_shape, (sample_chunk, num_steps), np.int32, compressor, args.zarr_format)
    cursor = 0
    t0 = time.time()
    last_t = t0
